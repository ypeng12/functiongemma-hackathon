...
def _normalize_local_calls(function_calls, tools):
...
def _rule_based_calls(messages, tools):
    ...
    calls = _normalize_local_calls(_dedupe_calls(calls), tools)
    if not calls:
        return {"function_calls": [], "confidence": 0.0}
...
def generate_hybrid(messages, tools, confidence_threshold=0.99):
    """
    Local-first strategy tuned for:
      1) high F1 (schema + intent coverage checks)
      2) high on-device ratio (rule path + lower local threshold)
      3) low latency (rule fast-path + cached model)
    """
    ...
    local["function_calls"] = _normalize_local_calls(local.get("function_calls", []), tools)

    if _should_accept_local(local, messages, tools, confidence_threshold):
        local["source"] = "on-device"
        return local

    # If local confidence is low but rule output is valid, use rule before paying cloud latency.
